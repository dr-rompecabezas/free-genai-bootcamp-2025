...
class ORBSitelenPonaRecognizer:
    ...
    def recognize(self, drawn_image, threshold=0.3, good_match_ratio=0.75):
        ...
        st.write(
            f"Input descriptor shape: {des1.shape if des1 is not None else 'None'}"
        )
        ...
        for char_name, template_data in self.processed_templates.items():
            ...
            if len(matches) > 0:
                ...
                if max_dist == min_dist:
                    ...
                    good_matches = [
                        m for m in matches if m.distance < good_match_ratio * max_dist
                    ]
    ...
    if mode == "Draw Character":
        ...
        if canvas_result.image_data is not None and st.button("Check Character"):
            ...
            if char:
                ...
                st.image(
                    recognizer.processed_templates[char]["image"],
                    caption=f"Reference {char}",
                    width=100,
                )
            else:
        ...
        if uploaded_file is not None:
            ...
            if st.button("Check Character"):
                ...
                char, confidence = recognizer.recognize(
                    image_array, threshold, good_match_ratio
                )
                ...
                if char:
                    ...
                    st.warning(
                        f"No match found (Best match confidence: {confidence:.2f})"
                    )
