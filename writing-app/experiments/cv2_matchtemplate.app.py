...
class SitelenPonaRecognizer:
    ...
    def compare_images(self, img1, img2):
        ...
        contours1, _ = cv2.findContours(
            img1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        contours2, _ = cv2.findContours(
            img2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    if mode == "Draw Character":
        ...
        if canvas_result.image_data is not None and st.button("Check Character"):
            ...
            if char:
                ...
                st.image(
                    recognizer.templates[char], caption=f"Reference {char}", width=100
                )
            else:
